...
export async function POST(request: NextRequest) {
  console.log('POST /api/nl-interface/update-entity-position - Processing entity position update from NL request');
  ...
  try {
    ...
    if (!entityId || !modelId) {
      return NextResponse.json(
        { error: 'Entity ID and Model ID are required' },
        { status: 400 }
      );
    }
    ...
    if (!finalPositionX || !finalPositionY) {
      return NextResponse.json(
        { error: 'Could not determine entity position' },
        { status: 400 }
      );
    }
    
    // Update entity position
    console.log(`Updating entity ${entityId} position to (${finalPositionX}, ${finalPositionY})`);
    ...
    if (updateError) {
      console.error('Error updating entity position:', updateError);
      return NextResponse.json(
        { error: `Failed to update entity position: ${updateError.message}` },
        { status: 500 }
      );
    }

    console.log('Entity position updated successfully:', updatedEntity);
    
    return NextResponse.json({ 
      success: true,
      entity: updatedEntity,
      message: `Successfully updated entity position`
    });
    
  } catch (error) {
    console.error('Error in POST /api/nl-interface/update-entity-position:', error);
    return NextResponse.json(
